...
METADATA_PATH = MODEL_DIR / "priority_model_metadata.json"
CONFUSION_MATRIX_PATH = PROJECT_ROOT / "docs" / "ml" / "priority_confusion_matrix.png"
RANDOM_STATE = 42
...
def load_it_tickets() -> tuple[list[str], list[str]]:
    ...
    priorities = [row["priority"] for row in it_rows]
...
def save_confusion_matrix(matrix: list[list[int]], class_names: list[str]) -> None:
    ...
    draw.text((10, 10), "IT priority confusion matrix", fill="black")
...
def main() -> None:
    ...
    METADATA_PATH.write_text(
        json.dumps(
            {
                "model_name": "IT support priority classifier",
                "model_version": "ml-v1",
                "target": "priority",
